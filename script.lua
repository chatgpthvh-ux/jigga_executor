...
function run(Character)
	...
	local SelectPlayer = Character.Name
	...
	local PlayerData = {
		[Character.Name] = Function.PlayerDataDefault()
	}
	...
	function Function.CharacterExecute(Character, Data)
		task.spawn(function()
			if Character then
				...
				pcall(function()
					for name, object in pairs(SelectedMorph.Cavities) do
				...
				pcall(function()
					for name, object in pairs(SelectedMorph.Compartments) do
	...
	if ExecutePlayer then
		local ExecuteCharacter = ExecuteCharacter
	...
	game:GetService("UserInputService").InputBegan:Connect(function(input,gpe)
		if not gpe then
			if input.KeyCode == Enum.KeyCode.C then
				PlayerData[Character.Name].UVToggle = not PlayerData[Character.Name].UVToggle
				Function.CharacterExecute(Character, Character.Name)
				warn("Toggled UV")
			end
			if input.KeyCode == Enum.KeyCode.RightBracket then
				PlayerData[Character.Name].CompartmentsScale = PlayerData[Character.Name].CompartmentsScale+ 0.2
				warn("Compartments set to "..	PlayerData[Character.Name].CompartmentsScale)
				PlayerData[Character.Name].SectionsScale = PlayerData[Character.Name].SectionsScale+ 5
				warn("Sections set to "..	PlayerData[Character.Name].SectionsScale)
				Function.CharacterExecute(Character, Character.Name)
			end
...
UIS.InputBegan:Connect(function(input, gameProcessed)
    ...
    if input.KeyCode == Enum.KeyCode.F then
        ...
        for _, model in ipairs(workspace:GetDescendants()) do
            if model:IsA("Model") and model:FindFirstChild("Humanoid") and model:FindFirstChild("HumanoidRootPart") then
                if model.Name == "iii_create543" or model.Name == LocalCharacter.Name then
                    table.insert(targets, model)
                end
            end
        end
