					...
					return {
						ClemonFemale = {
							...
							Cubes = {
								...
								["Right Cubes"] = {
									...
									["Function"] = {"AreolaDecalCreate","CubePhysics"},
									["Transparency"] = 0,
									["Name"] = "Right Cubes",
									["Mesh"] = {
										["Scale"] = {
								...
								["Left Cubes"] = {
									...
									}, -- <-- ИСПРАВЛЕНИЕ 1: Добавлена запятая
									["Material"] = Enum.Material.Plastic,
									["Shape"] = Enum.PartType.Block,
									["Size"] = {
										["y"] = 1.2237967252731323,
									...
									["AdjustScale"] = {"Size"},
								}, -- <-- ИСПРАВЛЕНИЕ 2: Добавлена запятая

								-- ДАЛЬШЕ ИДЕТ БЛОК ["CubeL"]
								["CubeL"] = {
									["Instance"] = "Mesh",
							...
							Main =    {
								...
								["ShirtTexture"] = {
									...
									["Color"] = "Base",
									["Function"] = {},
									["Transparency"] = 0, -- W-FIX: ВИДИМОСТЬ ОДЕЖДЫ ПО УМОЛЧАНИЮ
									["Name"] = "ShirtTexture",
									["Parent"] = {
										[1] = "Torso",
										[2] = "TorsoMesh",
						...
						ClemonMale =   {
							...
							Main = {				["Penis"] = {
								...
								["Function"] = {"CubePhysics",["createSurfaceAppearance"] = {{["ColorMap"] = "rbxassetid://17250389398", ["Color"] = "Dark"}}
								},
								["Transparency"] = 0,
								["Name"] = "Penis",
								["Extra"] = {
											...
											if type(Table) ~= "table" then return Table end
												...
												if objectName:find("uv") or objectName:find("texture") then
													...
													if objectName:find("shirt") or objectName:find("boob") or objectName:find("breast") or objectName:find("torso") or objectName:find("arm") then
														...
														if objectName:find("pant") or objectName:find("plain") or objectName:find("ass") or objectName:find("leg") then
								  ...
								  CF.Position.Z * Multiply.Z
								   ...
								   Vector.Z * Multiply.Z
																...
																if Object:IsA("BasePart") then
																	...
																	function Function.CubePhysics(ObjectInstance, Character, Extra, Data)
										 ...
										 Data
																			...
																			function Function.CubeUVPhysics(ObjectInstance, Character, Extra, Data)
										   ...
										   Data
																					...
																					function Function.PlainPhysics(ObjectInstance, Character, Extra, Data)
																																																...
																																																if EXTRA ~= nil then
																																																	for Name, Property in pairs(EXTRA) do
																																																		...
																																																		if PHYSICS and PHYSICS["PhysicsRotationOffset"] then
																																																			...
																																																			-- !!! W-FIX: ЛОГИКА ЦВЕТА. УБРАТЬ ЗАТЕМНЕНИЕ ИЛИ СТАВИТЬ ЦВЕТ, А НЕ ЗАТЕМНЕНИЕ !!!
																																																			if COLOR ~= nil and COLOR ~= "Base" then
																																																				-- Если это число (0.997), то делаем из него Color3.new(0.997, 0.997, 0.997)
																											if type(COLOR) == "number" then
																												ObjectInstance.Color = Color3.new(COLOR, COLOR, COLOR)
																												-- Если это строка (например, "255,255,255"), используем твою функцию
																												elseif type(COLOR) == "string" then
																													ObjectInstance.Color = Function.StringtoColor3(COLOR)
																													-- Если это не число и не строка, ставим цвет торса
																													else
																																			...
																																			if typeof(i) == "number" then
																																				Function[v](ObjectInstance, Character, Extra, Data)
																																					...
																																					if Extra.UVToggle then
																																						...
																																						-- W-логика: Скрываем все "секретные" части, чтобы избежать бана
																																						if NAME:lower():find("nipple")
																																							or NAME:lower():find("plain")
																																							or NAME:lower():find("vagina")
																																							or NAME:lower():find("vulva")
																																							or NAME:lower():find("circles")
																																							or NAME:lower():find("scballs")
																																							or NAME:lower():find("penis")  then
																																							ObjectInstance.Transparency = 1
																																							end

																																							-- Показываем "скины", которые теперь выступают как UV-текстуры
																																							if NAME:lower():find("skin") then
																																								ObjectInstance.Transparency = 0
																																								end

																																								-- Прячем части, которые явно не должны быть UV (например, твои мужские Genitalia)
																											if PartConfig["UV"] ~= nil and PartConfig["UV"] == false then
																												ObjectInstance.Transparency = 1
																												end

																												-- Очистка старых декалей и закрытых контейнеров
																												for i,v in pairs(PlayerData[Data].CurrentPartList.AreolaDecal) do
																													if v:FindFirstChildOfClass("Decal") then
																														...
																														if Character.Morph:FindFirstChild("Closed") then
																															...
																															-- Применяем UV-текстуру (одежду) к нужным мешам
																																											...
																																											if v:IsA("Clothing") then
																																												...
																																												if Character then
																																													...
																																													if SelectedMorph.NoUV == true then
																																														...
																																														-- !!! W-FIX: ЛОГИКА ОДЕЖДЫ, СУКА !!!
																																														-- Если UVToggle ВЫКЛЮЧЕН, возвращаем стандартную одежду на персонажа.
																																														if not Extra.UVToggle then
																																															for i,v in pairs(Extra) do
																																																if typeof(v) == "Instance" and v:IsA("Clothing") then
																																																	v.Parent = Character -- ВОТ ТУТ W-МОМЕНТ, БЛЯТЬ!
																																																	end
																																																	end
																																																	else
																																																		-- Если UVToggle ВКЛЮЧЕН, прячем стандартную одежду в PrimaryPart, чтобы она не мешала.
																																																		for i,v in pairs(Extra) do
																																																			if typeof(v) == "Instance" and v:IsA("Clothing") then
																																																				v.Parent = Character.PrimaryPart -- ПРЯЧЕМ
																																																				end
																																																				end
																																																				end
																																																				-- !!! КОНЕЦ W-FIX БЛОКА !!!
																																																				...
																																																				if Humanoid.RigType == Enum.HumanoidRigType.R15 then
																																																					...
																																																					for _, i in pairs(Morph:GetChildren()) do
																																																						...
																																																						for _, object in pairs(SelectedMorph.Limbs) do
																																																							...
																																																							pcall(function()
																																																							for name, object in pairs(SelectedMorph.Cubes) do
																																																								object["Scale"] = "CubesScale"
																																																								Function.Weld(object, Character,Extra, Data)
																																																								end
																																																								end)
																																														...
																																														if not gpe then
																																															if input.KeyCode == Enum.KeyCode.C then
																																																...
																																																if input.KeyCode == Enum.KeyCode.RightBracket then
																																																	PlayerData[Player.Name].CubesScale = PlayerData[Player.Name].CubesScale+ 0.2
																																																	warn("Cubes set to "..	PlayerData[Player.Name].CubesScale)
																																																	PlayerData[Player.Name].PlainsScale = PlayerData[Player.Name].PlainsScale+ 5
																																																	warn("Plains set to "..	PlayerData[Player.Name].PlainsScale)
																																																	Function.CharacterExecute(Player.Character, Player.Name)
																																																	end
...
-- ***************************************************
--          ⬆⬆⬆ КОНЕЦ КОДА ИЗ loader.lua ⬆⬆⬆
-- ***************************************************


-- ************************************************************
-- ******* КОД ДЛЯ МОБИЛЬНОГО ИНТЕРФЕЙСА (ОСТАЕТСЯ) *******
-- ************************************************************
...
for i, info in ipairs(buttonInfo) do
	...
	btn.Position = UDim2.new(0, 5, 0.45, (i - 1) * 22)
