...
-- Глобальная таблица для хранения данных всех игроков
...
if not Player then
    ...
    print("Applying morph to:", Player.Name)
            ...
            local check = {pcall(function()
                ...
                if (check[1]==false) then
                    ...
                    return nil
                        ...
                        -- Используем глобальную таблицу вместо локальной
                        if not GlobalPlayerData[Player.Name] then
                            GlobalPlayerData[Player.Name] = Function.PlayerDataDefault()
                            end

                            local PlayerData = GlobalPlayerData
                            ...
                            -- Вставьте сюда все остальные функции из вашего оригинального скрипта
                            ...
                            if ExecutePlayer then
                                ...
                                if ExecuteCharacter then
                                    ...
                                    -- Автоматически применяем при респавне
                                    ExecutePlayer.CharacterAdded:Connect(function(character)
                                    task.wait(1)
                                    Function.CharacterExecute(character, character.Name)
                                    end)
                                    end

                                    -- Глобальные подключения для физики (создаются только один раз)
                                    if not GlobalPlayerData._PhysicsConnected then
                                        GlobalPlayerData._PhysicsConnected = true

                                        local RenderConnect = RunService.RenderStepped:Connect(function()
                                        for PlayerName, DataList in pairs(GlobalPlayerData) do
                                            if type(DataList) == "table" and DataList.CurrentPartList then
                                                for Part, Property in pairs(DataList.CurrentPartList.RealtimeUpdateList.Mesh) do
                                                    ...
                                                    if GlobalPlayerData[PlayerName].CurrentPartList.BodyPartPhysics[Part] == nil then
                                                        Part.Size = Vector3.new(SIZE.x * XMultiply, SIZE.y * YMultiply, SIZE.z * ZMultiply)
                                                        Weld.C0 = CCFRAME
                                                        Weld.C1 = CCFRAME1
                                                        else
                                                            GlobalPlayerData[PlayerName].CurrentPartList.BodyPartPhysics[Part].CF = CCFRAME
                                                            GlobalPlayerData[PlayerName].CurrentPartList.BodyPartPhysics[Part].CF1 = CCFRAME1
                                                            end
                                                            end)
                                                    end
                                                    end
                                            ...
                                            if type(DataList) == "table" and DataList.CurrentPartList then
                                                for Part, Property in pairs(DataList.CurrentPartList.BodyPartPhysics) do
                                                    ...
                                                    if Function.FallenPartCheck(Part) then
                                                        GlobalPlayerData[PlayerName].CurrentPartList.BodyPartPhysics[Part] = nil
                                                        return
                                                        end

                                                        local Character = Players:FindFirstChild(PlayerName)
                                                                    ...
                                                                    for From, To in pairs(PositionOffset) do
                                                                        ...
                                                                        for From, To in pairs(RotationOffset) do
                                                                            ...
                                                                            if Property.Scale ~= nil and Property.Scale then
                                                                                ...
                                                                                GlobalPlayerData[PlayerName].CurrentPartList.BodyPartPhysics[Part].OriginCFrame = CurrentCFrame
                                                                                end
                                                                                end)
                                                    end
                                                    end
                                        ...
                                        print("Morph applied to:", Player.Name)
                                        end
